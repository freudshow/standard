...
/*共享内存允许两个或多个进程进程共享同一块内存(这块内存会映射到各个进程自己独立的地址空间) 
 从而使得这些进程可以相互通信。 
 在GNU/<a href="http://lib.csdn.net/base/linux" class='replace_word' title="Linux知识库" target='_blank' style='color:#df3434; font-weight:bold;'>Linux</a>中所有的进程都有唯一的虚拟地址空间，而共享内存应用编程接口API允许一个进程使 
...
int main()
{
	...
	//创建了一个4KB大小共享内存区段。指定的大小必须是当前系统<a href="http://lib.csdn.net/base/architecture" class='replace_word' title="大型网站架构知识库" target='_blank' style='color:#df3434; font-weight:bold;'>架构</a>  
	//中页面大小的整数倍  
	if (shmid > 0)
...
int main()
{
	...
	if (shmid >= 0) {
		...
		if ((int) mem != -1) {
			printf("Shared memory was attached in our address space at %p/n",
					mem);
			//向共享区段内存写入数据  
