...
class MainWindow(object):
    def __init__(self, parent=None):
        ...
        self.VERSION = '10.04.7'
        
        self.program_path = os.path.split( os.path.realpath( sys.argv[0] ) )[0]
        ...
        self.liststore_fstype.append( [ 'ext4', '（格式化为 ext4 ）' ] )
        self.liststore_fstype.append( [ 'ext3', '（格式化为 ext3 ）' ] )
        self.liststore_fstype.append( [ 'reiserfs', '（格式化为 reiserfs ）' ] )
        self.liststore_fstype.append( [ 'current', '（不格式化，使用现有文件系统 ）' ] )
    ...
    def on_button_estimate_clicked(self, widget, data ):
        ...
        self.init_sub_win()
        self.sub_win.set_title('估算体积')
        self.write_excludes_to_file()
        cmd = '%s/sh/estimate_size.sh'%self.program_path
        if data == 'clone':
    ...
    def on_button_refresh_clicked(self, widget ):
        ...
        self.liststore_partitions.append( ['', '不使用独立分区'] )
    ...
    def get_all_args(self):
        ...
        if fileName:
            self.backup_to = saveDir + '/' + fileName
        else:
            self.backup_to = ''
    ...
    def on_button_start_clone_clicked(self, widget ):
        ...
        self.sub_win.set_title('克隆')
        
        cmd = '%s/ucloner_cmd.py'%self.program_path
    ...
    def on_button_browse_backup_clicked(self, widget ):
        ...
        res = dialog.run()
        if res == gtk.RESPONSE_OK:
            self.entry_backup_dir.set_text( dialog.get_filename() )
        dialog.destroy()
